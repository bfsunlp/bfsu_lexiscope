...
def extract_http_urls(text: str) -> list[str]:
    """Extract unique http/https URLs from arbitrary text while preserving order.

    The helper is intentionally permissive so a user can import a plain text,
    Markdown, CSV, or TSV file containing one URL per line or URLs embedded in
    surrounding notes. Common sentence punctuation is trimmed from the end.
    """
    if not text:
        return []
